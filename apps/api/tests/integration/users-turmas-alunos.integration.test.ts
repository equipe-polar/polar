...
describe("Users, turmas e alunos", () => {
  ...
  it("lista usuarios sem vazar senha_hash", async () => {
    ...
    const response = await request(app).get("/usuarios").set("Authorization", `Bearer ${auth.adm}`).expect(200);
  ...
  it("permite ADM criar e editar turma", async () => {
    ...
    await request(app)
      .patch(`/turmas/${created.body.data.id}`)
      .set("Authorization", `Bearer ${auth.adm}`)
      .send({ turno: "Integral" })
      .expect(200);
  });
  ...
  it("bloqueia remocao de turma com aluno vinculado", async () => {
    ...
    await request(app).delete(`/turmas/${ids.turma}`).set("Authorization", `Bearer ${auth.adm}`).expect(409);
  });
  ...
  it("cria aluno apenas com turma valida e desativa aluno sem apagar historico", async () => {
    ...
    const removed = await request(app).delete(`/alunos/${ids.aluno}`).set("Authorization", `Bearer ${auth.adm}`).expect(200);
    expect(removed.body.data.ativo).toBe(false);

    const historico = await request(app)
      .get(`/ocorrencias/${ocorrencia.body.data.id}/historico`)
      .set("Authorization", `Bearer ${auth.professor}`)
      .expect(200);
