...
- [ ] Sistema no ar: `https://SEU-SERVICO.onrender.com/health` responde `ok`.
...
- [ ] Plano B: MySQL local via Docker + `pnpm dev` no notebook (ver docs/deploy/render.md), caso a internet da escola falhe.

## Credenciais de demonstração

| Papel | E-mail | Senha |
| --- | --- | --- |
| Professor | `professor@escola.demo` | (a definida em `SEED_SENHA_PADRAO`) |
| Coordenação | `coordenacao@escola.demo` | idem |
| Direção | `direcao@escola.demo` | idem |
| ADM | `admin@escola.demo` | idem |
...
1. **Login do professor** (`professor@escola.demo`). Mostrar que a lista se chama "Minhas ocorrências" — professor só vê o que ele registrou.
...
3. **Login da coordenação** (`coordenacao@escola.demo`): a ocorrência aparece; abrir o detalhe → **Colocar em análise**.
...
5. **Login da direção** (`direcao@escola.demo`) → **Encerrar** com observação.
...
- Professor B não enxerga ocorrência do professor A → **403**.
...
- **T10 ao vivo (opcional, ousado)**: reiniciar o serviço no painel do Render e mostrar que os dados continuam — "persistência real em MySQL".
...
| --- | --- | --- |
| "Que banco vocês usam?" | MySQL 8 na nuvem, utf8mb4, 11 tabelas normalizadas em 3FN | `database/schema.sql`, docs/banco-de-dados/ |
| "E se cair a conexão no meio da mudança de status?" | Transação: status + histórico + auditoria são atômicos (TCL) | `mysql/ocorrencia.repository.mysql.ts` |
