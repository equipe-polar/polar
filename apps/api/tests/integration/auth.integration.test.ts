...
describe("Auth", () => {
  ...
  it("recusa usuario inexistente e senha incorreta", async () => {
    const { app } = await buildTestContext();
    await request(app).post("/auth/login").send({ email: "nada@pola.test", senha: "Adm12345!" }).expect(401);
    await request(app).post("/auth/login").send({ email: "adm@pola.test", senha: "senha-errada" }).expect(401);
  });
  ...
  it("bloqueia usuario apos multiplas tentativas invalidas", async () => {
    ...
    await request(app).post("/auth/login").send({ email: "adm@pola.test", senha: "Adm12345!" }).expect(423);
  });

  it("recusa token ausente e token invalido", async () => {
    const { app } = await buildTestContext();
    await request(app).get("/usuarios").expect(401);
    await request(app).get("/usuarios").set("Authorization", "Bearer token-invalido").expect(401);
  });
});
